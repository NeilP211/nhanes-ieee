...
\begin{center}
{\large\bfseries Additional file 3: Table S1}\\[2pt]
...
TRIPOD+AI item 16. The cross-validated performance estimates reported in the main text come from the nested-CV procedure, in which each of the 50 folds fits its own pipeline; no single coefficient set exists from that procedure. This table is a separate refit of the identical pipeline on the complete analytic sample ($n=1{,}784$, 431 events), reported for TRIPOD compliance and not as a new performance estimate.

$C$ selected by 10-fold CV on the full sample: \textbf{0.1} (matches the mode across the original 50 CV folds, $C=0.1$ in 49 of 50). Intercept: $-1.1747$. Bootstrap: 2,000 resamples, 2,000 valid (re-selecting $C$ in each resample; a resample is excluded only if it produces a single-class sample or a different one-hot category set than the full-sample fit).

\textbf{Numeric features are standardised}, so each row is the odds ratio per one-SD increase, not per raw unit. \textbf{RIDRETH3 and DMDEDUC2 are one-hot encoded without a dropped reference category} (matching the modelling pipeline exactly, which relies on the L2 penalty rather than a reference contrast) --- these rows are not standard reference-category odds ratios and should not be read as such.

\footnotesize

\begin{longtable}{p{6.2cm}R{2.6cm}R{2cm}R{3.2cm}}
\captionsetup{labelformat=empty}
\caption{Table S1: Full M3 logistic regression: standardised coefficients, odds ratios, and 2,000-resample bootstrap 95\% confidence intervals.}
\label{tab:s1}\\
\toprule
Feature & Coefficient (standardised) & Odds ratio & 95\% CI \\
\midrule
\endfirsthead
\multicolumn{4}{l}{\textit{Table S1 continued}}\\
\toprule
Feature & Coefficient (standardised) & Odds ratio & 95\% CI \\
\midrule
\endhead
